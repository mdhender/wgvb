...
These are `const` data, computed at compile time, generated by the same `rotate_cw` used for player rotation (appendix A):
...
is a pure coordinate operation, and `rotate_cw` is required there anyway to
...
7.1 is the six rotations of `(2N+1, -N, -N-1)` under `rotate_cw`, so write the
...
```text
absolute = normalize( rotate_cw^k ( relative ) + player_origin )
